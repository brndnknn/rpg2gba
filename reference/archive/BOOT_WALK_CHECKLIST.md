...
- [X] **S6b** — Rival (Theo) battle after the aptitude test **(BUILT — ROM `51be1e63`, 2026-07-21)**: after picking up your starter, Theo challenges you ("...we hafta battle!") and a real trainer battle fires **in the lab** vs Theo's counter-pick starter (the type that beats yours: you Orchynx→Theo Eletux, Raptorch→Orchynx, Eletux→Raptorch, all L5). It's a **losable tutorial battle** — if you lose, the scene continues anyway; either way your party **auto-heals** afterward. Then Theo **walks off and leaves** (fixes the old stranded-sprite bug), the professor gives his "meet me at the Route 01 entrance, bring Theo" line and leaves, and the chain advances. **Known cosmetic gap — do NOT report:** Theo's in-battle sprite is a stock Emerald **Youngster** stand-in (no Uranium trainer-pic pipeline in slice 1). *Check: battle fires, both win AND lose continue cleanly (no white-out/soft-lock), party heals, Theo is gone from the lab afterward.*
- [ ] **S7** — PokéPod scene in Theo's house 1F: fires on entry after the lab visit, advances the quest chain (var 101 → 2). **Theo-invisible fix — ROM `8fe9d9ff`, 2026-07-21:** the scene fired but Theo was invisible on the win path — the flag-hidden-actor auto-bracket placed its `addobject(Theo)` only in the `if (FLAG_LOST_FIRST_BATTLE)` branch, so the `else` (win) path ran `set_visible` on an unspawned actor. `hidden_actor_bracket.py` is now branch-aware (spawns the actor on every execution path). *Check: Theo is visible for the Cameron/PokéPod scene whether you won OR lost the S6b battle.*
