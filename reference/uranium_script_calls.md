...
>
> This covers the ~40 highest-frequency signatures (≈80% of the 7,961 script-call
> lines). The long tail (~210 signatures) is intentionally undocumented: the agent
> must queue anything not listed here. Tags are **advisory** and get human review
> at the §9 #2 calibration gate.
...
- `(non-identifier)` and bare lowercase tokens (`i`, `x`, `p`, `poke`, `pkmn`,
  `result`, `stat`, …) are assignment targets / loop variables inside a larger
  expression. Read the full line in the event JSON before deciding; if the whole
  statement has no game-state effect, STRIP it; if it does but has no equivalent,
  queue the statement (not the token).
