...
**Active phase:** Phase 2 — PBS Data Conversion (§2.0 scaffolding ✓; **§2.1 species COMPLETE**; **§2.2 moves COMPLETE**; **§2.3 items COMPLETE**; **§2.4 abilities COMPLETE** — parse + emit + tests; §2.5 TM/HM next)
...
**Next concrete task:** §2.5 TM/HM — `pbs_converter/tm_hm.py`. Source `tm.dat` + `tutor.dat` (indexed binary, single uint16 IDs per entry). Verify `tm.dat` byte direction (TM→species vs species→TM) against Compiler.rb first. **Note:** the fork's `.teachableLearnset` is BUILD-GENERATED by `tools/learnset_helpers/make_teachables.py` — do NOT hand-emit `sXTeachableLearnset` arrays; feed TM/tutor compat into that generator (see Key File Notes P4 gotcha). `tutor.dat` ships empty (§2.1 finding). Mirror the focused `_Resolver` pattern. PHASE2_PLAN.md is the canonical task list — tick `- [ ]` boxes as items land.
...
*Older summaries (05-20 §2.2 moves, 05-20 §2.1 species, 05-18 ×2, 05-12, 05-09) are archived in `reference/memory-archive.md`.*
