...
// BEGIN URANIUM PATHFINDER SLICE — pre-planted berry tree seed table (S8d)
#include "constants/berry.h"
// Quote-include search for a .c file starts at the FILE's own directory
// (src/), unlike the .s hook in data/event_scripts.s (built with -I "",
// i.e. engine root) that includes uranium_flags.h/uranium_map_aliases.h
// directly as "data/scripts/...". Hence "../" here to reach engine/data/
// scripts/ from src/ (mirrors how src/data/*.h siblings resolve "data/...").
#include "../data/scripts/uranium_berry_trees.h"
// END URANIUM PATHFINDER SLICE
...
void NewGameInitData(void)
{
    ...
    ClearBerryTrees();
    // BEGIN URANIUM PATHFINDER SLICE — pre-planted berry tree seed table (S8d).
    // Route 1's outer two berry patches (Uranium: Oran berry trees you can pick
    // immediately) start fully grown; the middle patch stays bare soil and is
    // simply never listed here. allowGrowth=FALSE stops the tree at
    // BERRY_STAGE_BERRIES until the player has seen it (berry.c PlantBerryTree
    // tail comment) — exactly the "already there, don't regrow" semantic we want.
    // uranium_berry_trees.h is generated by scripts/assemble_pathfinder.py
    // (run_berry_tree_pass); DO NOT hand-edit.
#define URANIUM_PLANT_BERRY_TREE(treeId, berryItem)                         \
    if ((treeId) < BERRY_TREES_COUNT)                                       \
        PlantBerryTree((treeId), ItemIdToBerryType(berryItem),              \
                        BERRY_STAGE_BERRIES, FALSE);
    URANIUM_BERRY_TREE_SEED_LIST(URANIUM_PLANT_BERRY_TREE)
#undef URANIUM_PLANT_BERRY_TREE
    // END URANIUM PATHFINDER SLICE
